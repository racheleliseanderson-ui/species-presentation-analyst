...
/*
 * A test here read every recipe in .grok/skills/og/references/ and checked
 * that each `node scripts/write-atomic.mjs` line it printed was one this CLI
 * would accept. That directory belonged to the Grok app-builder scaffold and
 * is gone; the check has nothing left to read. The CLI's own argument handling
 * is covered directly above and below.
 */

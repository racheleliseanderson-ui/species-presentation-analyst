...
The Species Profile is the optional `AFP-1.3` reference layer: the same ten angler questions as `AFP-1.0`. Identification, behavior, diet, seasonal calendar, fight, and food value can be marked reviewed when `AFP-ID-1.0` / `AFP-BH-1.0` / `AFP-DI-1.0` / `AFP-SC-1.0` / `AFP-FT-1.0` / `AFP-FV-1.0` dossiers exist. Wave 01 covers 26 high-confusion species through food value. Wave 02 adds identification and behavior for 15 open-first salmonids, pike, walleye, large catfish, and yellow perch; diet, calendar, fight, and food on those records stay as visible gaps. `AFP-Q-1.0` ranks the remaining 34 into queued research waves instead of filling them with generic model text. Fight is descriptive, never a score. Food is table character, never a consumption-safety claim. Dossiers do not feed presentation weighting.

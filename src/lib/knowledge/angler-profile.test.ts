...
test("AFP-1.2 uses identification dossiers when reviewed and leaves remaining species explicitly incomplete", () => {
  ...
  assert.ok(rainbow && chinook);

  const rainbowProfile = buildAnglerSpeciesProfile(rainbow);
  const chinookProfile = buildAnglerSpeciesProfile(chinook);
  const rainbowId = rainbowProfile.sections.find((section) => section.id === "identification");
  const chinookId = chinookProfile.sections.find((section) => section.id === "identification");
  ...
  assert.equal(chinookId?.status, "partial");
  assert.ok(chinookId?.gaps.includes("similar-species comparison keys"));
});
...
test("AFP-1.2 uses behavior dossiers when reviewed without converting them into catch claims", () => {
  ...
  const chinookBehavior = chinookProfile.sections.find((section) => section.id === "behavior");

  assert.equal(whiteBehavior?.status, "reviewed");
  assert.ok(whiteBehavior?.facts.some((fact) => fact.label === "Social pattern"));
  assert.equal(chinookBehavior?.status, "partial");
  assert.ok(chinookBehavior?.gaps.includes("schooling versus solitary behavior"));
...
test("AFP-1.2 uses diet dossiers when reviewed and does not infer a current hatch", () => {
  ...
  const chinookDiet = chinookProfile.sections.find((section) => section.id === "diet");
  ...
  assert.equal(chinookDiet?.status, "partial");
  assert.ok(chinookDiet?.gaps.includes("spring / summer / fall / winter diet shifts"));
});
...
test("AFP-1.2 uses seasonal calendars when reviewed without turning spawn into a target map", () => {
  ...
  const chinookCal = chinookProfile.sections.find((section) => section.id === "seasonal_calendar");
  ...
  assert.equal(chinookCal?.status, "partial");
  assert.ok(chinookCal?.gaps.includes("month-by-month location changes"));
});

      ...
      <div className="stagger-in space-y-6 no-print">
        ...
        <ThermalBandPlate
          ...
          aside={
            <p>
              ...
              published range describes a population, and you are standing at one point in one
              system on one day.
            </p>
          }
        />
              ...
              {i === 0 ? (
                <>
                  {result.presentations[1] && (
                    <p className="mt-4 text-sm text-muted">
                      ...
                      {result.presentations[1].label} remains plausible if that declaration is
                      wrong.
                    </p>
                  )}
                ...
                <dl className="mt-4 grid grid-cols-1 gap-2 text-xs sm:grid-cols-3">
                  {COMPARE_AXES.filter((axis) => p.system[axis.key]).map((axis) => (
                    <div
                      key={axis.key}
                      className="rounded-[var(--radius-xs)] bg-subtle px-2.5 py-2"
                    >
              ...
              {drivers.map((d) => (
                <li key={`${d.variable}-${d.to}`} className="text-sm text-fg">
                  ...
                  <span className="text-muted">
                    {" "}
                    {d.from} → {d.to} moves the leading family from {d.familyBefore} to{" "}
                    {d.familyAfter}.
                  </span>
                </li>
              ))}
        ...
        <section className="no-print rounded-[var(--radius-lg)] bg-elevated p-6 shadow-[var(--shadow-border)] sm:p-8">
          ...
          <p className="mt-2 text-sm text-muted">
            Nothing leaves this device unless you send it, and what travels carries no coordinates
            and no bite score.
          </p>
          <div className="mt-4 flex flex-wrap gap-2">

    ...
    <div className="min-h-dvh bg-bg text-fg">
      ...
      <main className="mx-auto max-w-3xl px-4 py-12 sm:px-6">
        <p className="font-mono text-[11px] uppercase tracking-[0.18em] text-mark">
          The gaps are printed on the dial
        </p>
        <h1 className="mt-4 font-display text-5xl">What this won't tell you</h1>
        <p className="mt-5 text-base text-muted">
          Species & Presentation Analyst explains biological plausibility. It does not compete with
          catch-prediction products. The list below is a chosen editorial stance, not a shortcoming
          waiting to be fixed. These limits are product rules, not preferences.
        </p>
        <ul className="mt-8 space-y-3">
          {REFUSES.map((r) => (
            <li
              key={r}
              className="instrument-rule rounded-[var(--radius-md)] bg-elevated px-5 py-4 text-sm"
            >
              {r}
            </li>
          ))}
        ...
        <p className="mt-8 text-sm text-muted">
          No unreviewed species falls through to generic AI-written text. With no reviewed record
          for a fish, the app says so and stops.
        </p>

        <section
          className="mt-12 border-t border-line pt-10"
          aria-labelledby="catalog-knowledge-heading"
        >
          <p className="font-mono text-[11px] uppercase tracking-[0.18em] text-mark">
            Catalog knowledge
          </p>
          ...
          <p className="mt-4 text-sm text-muted">
            Enrichment is a ranked queue of lookalike groups, not a race to fill seventy-five
            encyclopedia pages. A species becomes knowable only when identification, behavior, diet,
            and seasonal calendar are reviewed together from agency or peer-reviewed sources.
          </p>
          {coverage.status === "ready" ? (
            <p className="mt-5 text-sm text-fg">
              {coverage.coverage.completeOverlays} of {coverage.speciesTotal} species have that full
              overlay set. Identification {coverage.coverage.identification} · behavior{" "}
              {coverage.coverage.behavior} · diet {coverage.coverage.diet} · season{" "}
              {coverage.coverage.seasonal_calendar}. Fight and food value stay unreviewed. Live
              limits stay outside the static catalog.
            </p>
          ) : (
          ...
          <p className="mt-6 text-xs leading-5 text-dim">
            {SEED_DOCTRINE.unit} {SEED_DOCTRINE.usefulFirst} We do not fill remaining species with
            model text. Deferred until the high-use set is knowable: fight, table quality, gear
            ranges, and live regulations.
          </p>
        </section>
